...
7. `container-arm64`は公開repositoryのnative `ubuntu-24.04-arm`でproduction/fault-test/break-glass targetをbuildする。full SHA固定した`docker/setup-buildx-action`と`docker/build-push-action`を使い、3 targetを`outputs: type=docker,rewrite-timestamp=true`で同じnative Docker daemonへ読み込む。全buildへ`SOURCE_DATE_EPOCH=0`を渡してindex/config/historyの時刻を固定し、Dockerfileのglobal `ARG`をbuilderで再宣言して環境へ公開することで、`UV_COMPILE_BYTECODE=1`が作る`.pyc`をtimestamp型ではなくhash型へ固定する。productionとbreak-glassのimage config digestをrisk acceptance identityとして記録し、releaseも別のregistry/image exporterで再buildせず同じDocker exporter、`rewrite-timestamp=true`、target別のmain CI cache scopeを使ってbuild/loadする。releaseは両方のloaded configをpolicyへ照合するまでECRへpushせず、合格した同一local imageだけを各1回pushし、ECR manifestのconfigが事前値と一致することを再検証する。production buildは`container-arm64-production`、break-glass buildは`container-arm64-break-glass` scopeへ`mode=max`のGHA cacheをexportし、fault buildは同じjob内のBuildx builder cacheを再利用する。ただしconfig identityを構成するvenvは独立再利用される`COPY --link` layerにせず、最終stageへ通常の`COPY --from=builder`で取り込む。CIとReleaseのrisk-bound 2 imageでは、`builder`と各最終stage（productionは`runtime-base`、break-glassは`break-glass`）を`no-cache-filters`で毎回再生成する。GHA cacheはbase image等の非対象stageだけに利用し、過去runが圧縮済みbuilder snapshotを再利用する経路を持たせない。builderは`pyproject.toml`と`uv.lock`だけをcopyして`uv sync --frozen --no-dev --no-install-project --no-editable`を実行した後、application sourceをcopyしてprojectをinstallする。final sync後はPyPAの三列CSV契約を維持したまま、全`.dist-info/RECORD`をrow全体でsortしてLFへ正規化する。さらにuvがlocal project install時に生成するcheckout時刻依存の`uv_cache.json`本体と対応する同一dist-info RECORD行だけを除去し、別pathのuv cache行、symlink、非regular fileはfail closedで拒否する。最後に`.venv`配下のregular file、directory、symlinkすべてのmtimeを`SOURCE_DATE_EPOCH`へ固定し、未知file typeと正規化不能なentryをfail closedで拒否する。これによりcold installとcache reuseでapplication RECORD、uv内部cacheまたはfilesystem mtimeだけが変わり、`.venv` layer/config digestが変化する経路を閉じる。CIはproduction imageからapplication RECORDを再取得し、非空かつ`uv_cache.json`行がないことを検証してSBOM・両config digestと同じ30日artifactへ保持する。`/root/.cache/uv`は`sharing=locked`のBuildKit cache mountとし、runtime imageへcopyしない。既定Python image以外の取得を`UV_PYTHON_DOWNLOADS=0`で禁止する。image config、read-only/non-root/capability、health、SIGTERM/SIGKILL recoveryを検査し、`.github/tool-versions.json`でSHA-256固定したSyft（現行pin v1.49.0）でproduction/break-glass双方のOS/runtime dependencyを含むSPDX JSONを生成して30日保持する。secret、OIDC、registry push、paid/network integrationは使用しない。
...
Docker exporterはcache hitとcache missで既存の圧縮blobと新規圧縮blobを使い分けてはならない。CIのproduction、fault-test、break-glassとReleaseのproduction、break-glassは、`type=docker`と`rewrite-timestamp=true`に加え、`compression=gzip`、`compression-level=6`、`force-compression=true`を共通のexporter条件とし、全5 buildへBuildKit builtin build argument `BUILDKIT_MULTI_PLATFORM=1`を渡してplatform数に依存しない決定的なexport resultを要求する。risk-bound imageでは`builder`とdeterministic venv transferを行う最終stageをともにstage限定no-cacheへ含める。圧縮前の同一`diff_id`と同一logical image configでraw blob/config digestだけが変化した場合、baseline更新を繰り返さずBuildKit export resultの決定性を先に是正する。
