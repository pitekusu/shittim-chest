...
7. `container-arm64`は公開repositoryのnative `ubuntu-24.04-arm`でproduction/fault-test/break-glass targetをbuildする。full SHA固定のBuildx Actions、`SOURCE_DATE_EPOCH=0`、`type=docker,rewrite-timestamp=true,compression=gzip,compression-level=6,force-compression=true`をCIとReleaseで共通化する。risk-bound 2 imageでは`builder`と各最終stageを`no-cache-filters`で毎回再生成し、productionのvenvはruntime Python helperでpath順、content、symlink、numeric owner/group、directoryを含むmode、mtimeを明示固定して取り込む。`COPY --from=builder`と`COPY --link`は使用しない。builderはlock済みdependencyとapplicationを順にinstallし、RECORD、`uv_cache.json`、全venv mtimeをcanonical化する。CIはproductionのRECORD、両imageのconfig digest、全RootFS diff ID、SBOM、VEXを同じ30日artifactへ保持し、risk gateとの対応を検証する。Releaseはpush前に両configをpolicyへ照合し、push後もECR manifestのconfig一致を再検証する。secret、OIDC、registry push、paid/network integrationはCIで使用しない。
