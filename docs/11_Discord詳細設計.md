...
5. thread ID、starter message ID、control panel message IDをDynamoDBへ保存する。
...
- Retry: 開始userまたは`Manage Messages`保持者、かつ`FAILED`だけ許可。
- 永続化済みpanel operation ID、Guild、thread、message、debate IDのいずれかが一致しない操作はephemeral拒否する。独自署名方式は導入しない。
...
- outboxへBot ID、nonce、content hash、chunk sequenceを保存してから送信する。
