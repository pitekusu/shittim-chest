...
STEP-05AはPR `#20`、STEP-05BはPR `#21`、STEP-05CはPR `#22`でmerge済みである。STEP-05CはPolicy request shape、shadow判定、content-free Policy telemetry、opt-in blind評価toolを実装した。STEP-05C.1AはPR `#24`、merge commit `1360411`で盲検artifact分離、failure capture、rubric validation、content-free集計を実装・検証済みである。STEP-05C.1Bは10件20回答の実API生成とpreference-only集計を完了し、Luna pro 4勝、Terra standard 2勝、同点4件となった。Luna proは実測候補に留め、本番自動昇格、threshold、token/deadline上限、Discord結合、CloudWatch出力は未実施・未決定である。
