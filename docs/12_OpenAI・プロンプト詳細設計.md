...
判断準備・候補選択から初回意見・候補調整・再考・最終案・勝者発表まで、共通の自己認識規則を渡す。
会話中の正体や意思への問いには、汎用モデルの自己紹介ではなく、選択中のキャラクターの
一人称・感情・判断で応じる。判断準備でも技術仕様の正解を答える課題に置き換えず、
他の参加者がモデルとして自己紹介しても、その発言へ同調して役柄を離れない。

キャラクター設定上のAIと、サービスを動かす生成モデルは区別する。AIであることの機械的な否定や、
実在人物本人であるとの主張は指示しない。利用者が明確に演技の外から実装・モデルを確認する場合は、
役柄と実装を区別し、渡された情報の範囲で正確に説明する。モデルの詳細版や秘密の設定は推測・開示しない。

この規則はコード所有であり、管理画面の人格本文・active revisionは変更しない。
事前調査、親愛度評価、匿名投票の事実境界・採点基準には追加せず、API呼出回数も増やさない。
...
| 初回意見 | Pythonが採用した第1候補を本人の言葉で表現し、一般的なおすすめを選び直さない | high・2,400 tokens（上限到達時のみ4,800） |
...
判断基準・候補選択・初回意見では、終了理由が`max_output_tokens`と確認できた場合だけ、同じアダプター呼び出し内で
出力上限を2倍にして最大1回再生成する（判断基準4,000、候補8,000、初回意見4,800 tokens）。部分出力は破棄し、
...
| モモトーク | — | — | 週次生成開始時点の人格 | 直前までの会話 | 週次生成開始時点の親愛度に応じた感想 |
| Inspector脆弱性の翻訳 | — | — | — | — | — |
...
| 帰宅挨拶 | `FarewellOutputV2` | 挨拶文。出典は提供元の引用注釈、なければ検索元URLから別に取得 |
...
- 有効なHTTP(S)出典が1件以上ある場合に成功とし、最初のURLを「参考リンク」として別行に付ける。
  引用注釈を優先し、注釈がない場合は同じ応答の`web_search_call.action.sources`にある
  `type=url`の検索元を使う。検索完了と本文の検証は引き続き必須とし、本文中のURLや
  URLのない天気フィードだけでは代替しない。ユーザー情報入り・不正なURLは採用しない。
  注釈0件でも検索元URLがあれば再生成せず投稿する。注釈件数のログは0件のまま維持する。
...
週刊チャットの独立人格要求、100書記素上限、画像条件と再開処理は[モモトーク設計](28_モモトーク設計.md)を参照する。
