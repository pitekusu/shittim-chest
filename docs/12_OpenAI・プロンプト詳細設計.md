...
### 新規候補探索と公開回答の再考（比較用v2）

2026年9月8日の比較用改修。既存の比較経路を上書きせず、
`tools/reconsideration_v2.py`へ分離している。本番の状態遷移へはまだ組み込まない。
...
| 順序制御 | 候補調整は最大2人格、回答後の再考は最大3人格。順番に処理し、後続は直前までの変更を参照する。先頭はケースごとに交代する |
