...
- 是正では、投票・winner計算・永続schemaの正本である`ParticipantSlot`を変更しない。起動時に検証済み`PersonaConfig.display_name`の3件完全mappingをcomposition rootからapplicationへ注入し、3票確定後のDiscord Outbox生成時だけ候補slotを表示名へ変換する。実表示名はrepositoryや文書へ直書きせず、欠落・余分・空表示名、およびDiscord rendererが拒否するUnicodeを含む表示名はSDK client作成前にfail closedとする。
