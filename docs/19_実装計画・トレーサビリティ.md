...
| Androidへの認証受け渡し（C05〜C06） | [29](29_Androidアプリ設計.md) | `mobile_auth.py`、`mobile_auth_adapters.py`、`mobile-auth.schema.json`（公開前の契約・保存部品） | 入力・期限・状態・秘密非表示、DynamoDB Localの同時更新・二重消費・原子的失敗。認証実行／公開接続はC07以降 |
