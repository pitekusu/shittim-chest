...
| Androidへの認証受け渡し（C05〜C11） | [29](29_Androidアプリ設計.md) | `mobile_auth.py`、`mobile_auth_adapters.py`、`mobile_login.py`、`mobile_callback.py`、`mobile_exchange.py`、`mobile_session.py`、`http_api.py`、`mobile-auth.schema.json` | 入力・期限・state／Cookie・Discord本人確認・S256・90日の絶対期限・token単位の失効・Cookie/Bearer混在拒否・議論GET限定・秘密非表示、DynamoDB Localの同時更新・二重消費・原子的失敗・削除対象の分離。認証ルート公開はC12 |
