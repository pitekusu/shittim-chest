...
| Androidへの認証受け渡し（C05〜C10） | [29](29_Androidアプリ設計.md) | `mobile_auth.py`、`mobile_auth_adapters.py`、`mobile_login.py`、`mobile_callback.py`、`mobile_exchange.py`、`mobile_session.py`、`mobile-auth.schema.json`（公開前の契約・保存・セッション管理） | 入力・期限・state／Cookie・Discord本人確認・S256・90日の絶対期限・token単位の失効・秘密非表示、DynamoDB Localの同時更新・二重消費・原子的失敗・削除対象の分離。読取API接続はC11、公開接続はC12 |
