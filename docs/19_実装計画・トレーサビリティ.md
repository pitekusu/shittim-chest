...
| Androidへの認証受け渡し（C05〜C08） | [29](29_Androidアプリ設計.md) | `mobile_auth.py`、`mobile_auth_adapters.py`、`mobile_login.py`、`mobile_callback.py`、`mobile-auth.schema.json`（公開前の契約・保存・コード発行） | 入力・期限・state／Cookie・Discord本人確認・秘密非表示、DynamoDB Localの同時更新・二重消費・原子的失敗。セッション発行はC09、公開接続はC12 |
