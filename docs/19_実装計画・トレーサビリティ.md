...
| [apps/records-android](https://github.com/pitekusu/shittim-chest/tree/main/apps/records-android) | Androidネイティブアプリ。[29](29_Androidアプリ設計.md)のCompose・Expressive・Circuit／Metro、C03 CI、C13トークン保存、C14認証API、C15 Auth Tab、C16認証画面、C17記録1件表示、C18 Release設定。C04は保留 |
...
| 品質観測 | [10](10_アプリケーション・Python詳細設計.md)、[12](12_OpenAI・プロンプト詳細設計.md) | [escalation.py](https://github.com/pitekusu/shittim-chest/blob/main/src/shittim_chest/domain/escalation.py)、[observability.py](https://github.com/pitekusu/shittim-chest/blob/main/src/shittim_chest/adapters/openai/observability.py) | 本番は`luna_standard`で実行し、本文を含まない品質シグナル・使用量・失敗を記録 |
...
人格別の判断基準・候補選択は[10](10_アプリケーション・Python詳細設計.md)、
[12](12_OpenAI・プロンプト詳細設計.md)、[13](13_DynamoDB・データ整合性詳細設計.md)を正とする。
Coreの`domain/deliberation.py`、アプリケーション、OpenAIアダプターと保存処理を対応させ、
`test_deliberation.py`、DynamoDB Local、RecordsのArchive非公開検査で境界を確認する。
`tools/evaluate_deliberation.py`の有料比較は通常CIから分離する。

再考・候補調整v2は`application/reconsideration.py`と`adapters/openai/reconsideration.py`を
本番の進行処理へ接続する。単体・SDK契約の`test_reconsideration.py`とDynamoDB Localで、
逐次更新、同一結論の許容、投稿前の完了、途中再開、リース・進捗の競合防止を確認する。

## Web・親愛度・メモリアル

モモトークの正本は[28 モモトーク設計](28_モモトーク設計.md)。Recordsの`momotalk*.py`が集計・生成・保存・公開境界を、
Webの`MomotalkPage.tsx`が閲覧と再生を担当する。`test_momotalk*.py`、DynamoDB Local、`momotalk.spec.ts`で境界と操作を確認する。

| 要求 | 設計の正本 | 主な実装 | 主な確認 |
|---|---|---|---|
| 議事録への投影・過去分取り込み | [24](24_シッテムの箱%20議事録設計.md) | Recordsの`archive.py`、`projector.py`、`adapters.py` | 保存形式、勝者再検証、重複・逆順イベント、ログへの本文非出力 |
| 完了記録のDiscordリンク | [24](24_シッテムの箱%20議事録設計.md)、[14](14_AWS・CDK詳細設計.md) | Recordsの`record_link_notifications.py`、`record_link_adapters.py`、Projector | Archive保存後の元チャット投稿、再試行の重複防止、Backfill非投稿、権限分離 |
| 議論詳細OGPと投稿前準備 | [24](24_シッテムの箱%20議事録設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | Recordsの`ogp.py`、`ogp_render.py`、`ogp_http.py`、`ogp_adapters.py`、`ogp_handler.py` | 公開項目限定、PNG保存後のリンク通知、20秒上限と失敗継続、画像再利用、HTML／SPA、呼び出し権限の完全ARN一致 |
| 認証・セッション・閲覧 | [24](24_シッテムの箱%20議事録設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `auth.py`、Authlibを利用するDiscordアダプター、Cookieセッション／Origin・CSRFの共通検証、`read_api.py`、`http_api.py` | OAuth要求・応答、状態の一回限り使用、期限、Origin/CSRF、既存エラー契約、機能別認可、非公開識別子の遮断 |
| Androidへの認証受け渡し（C05〜C12） | [29](29_Androidアプリ設計.md) | `mobile_auth.py`、`mobile_auth_adapters.py`、`mobile_login.py`、`mobile_callback.py`、`mobile_exchange.py`、`mobile_session.py`、`mobile_http.py`、`http_api.py`、`mobile-auth.schema.json`、OpenAPI、RecordsApplication | 入力・期限・state／Cookie・Discord本人確認・S256・90日の絶対期限・token単位の失効・Cookie/Bearer混在拒否・議論GET限定・秘密非表示。架空の認証縦断、DynamoDB Localの原子更新、5公開ルート、匿名401 smoke |
| Androidの端末内トークン保存（C13） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `StoredToken.kt`、`KeystoreTokenStore.kt`、既存Manifest・バックアップ除外XML | 実Keystoreで暗号化・再読込・改ざん拒否・鍵消失・限定削除・書込失敗・秘密非表示。画面・API接続は後続 |
| Androidの認証APIクライアント（C14） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `MobileAuthModels.kt`、`MobileAuthClient.kt` | 架空データの型検証とMockEngineで4操作・失効／通信失敗の区別・転送拒否・Cookie非使用・応答上限・キャンセル・秘密非表示。ブラウザー・画面は後続 |
| Androidのブラウザー認証（C15） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | Auth Tab、`MobileLoginActivity.kt`、`MobileLoginFlow.kt`、C13／C14 | Auth Tabとfallbackの結果処理、S256公開vector、復帰URL・state・期限・キャンセル・古いコード・二重交換・応答不明・保存を架空データで確認。実署名のApp Links確認はC19 |
| Androidの認証画面（C16） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `MobileSessionModel.kt`、`SessionPanel.kt`、Circuit／Metro、C13〜C15 | 二重起動・期限／401・通信障害・削除失敗・ログアウト・アカウント切替。画面操作と本人情報の非表示、エミュレーターの明暗・狭幅／文字拡大 |
| Androidの記録1件表示（C17） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `RecordsReadClient.kt`、`RecordPreviewPanel.kt`、C16の認証状態 | 一覧→詳細・空・個別復帰先・ID／winner不一致・401、ログアウト後の本文非表示、エミュレーターの狭幅／文字拡大 |
| AndroidのRelease設定（C18） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `apps/records-android/app/build.gradle.kts` | debugとのapplication ID分離、版番号入力、秘密値なしのRelease拒否、合成鍵での署名済みAAB確認。実鍵・Play配布・App LinksはC19以降 |
| Androidのオフライン認可・端末削除（C30） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `MobileSessionModel.kt`、`StoredToken.kt`、`KeystoreTokenStore.kt`、`RecordCacheAccount.kt`、`RecordsRepository.kt` | v1読込互換・v2改ざん拒否、検証済み本人・90日境界・再起動・失効・同一アカウント再認証。ログアウト時の鍵・全行削除と削除失敗の停止。オフライン画面はC31〜C32 |
| Androidの全記録同期（C31） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `RecordSyncCheckpoint.kt`、`RecordSynchronization.kt`、`RecordsRepository.kt`、`RecordSyncStatus.kt`、Circuit・Coroutines・Lifecycle Compose | 全ページ・重複・未取得詳細のみ取得、暗号化進捗・再オープン・キャンセル・cursor期限／巡回・404・認可喪失。同期状態・明暗・320dp／文字2倍。当初の手動同期UIはC32で自動同期へ移行。オフライン表示・更新統合はC32 |
| Androidの保存済み表示と更新（C32） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `RecordsRepository.kt`、`RecordSynchronization.kt`、`BootstrapPresenter.kt`、一覧・詳細パネル、`MobileSessionModel.kt` | 期限内オフライン閲覧・戻る・失効後の非表示。更新失敗・DB書き込み拒否時の旧記録保持、認証済み404、全列挙成功後のみ削除。明暗・320dp／文字2倍 |
| Androidの自動差分同期・アイコン保存（C32追加） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `RecordSyncWorker.kt`、`RecordSyncScheduler.kt`、`RecordSyncStatus.kt`、既存暗号化Store、`read_api.py`・`http_api.py`・契約・RecordsApplicationの同期索引 | 本文の新規／更新分のみ取得、アイコン共用・暗号化・オフライン読込、Workerの期限／削除意図／本人検証、途中再開、全索引成功後の不在候補は404確認時だけ削除。手動同期UIなし、同期中／失敗のみ小さく表示。新API先行配信後にAndroidを配布 |
| Android・関連Pythonのライブラリ優先方針 | [29「高レベルAPI・ライブラリ優先の開発方針」](29_Androidアプリ設計.md#高レベルapiライブラリ優先の開発方針) | `AGENTS.md`と29の用途別採用表。C15までの実装を維持し、C16以降の依存は各C工程で導入 | 文書・mirrorの整合。導入時に互換性・安全設定・接続境界を確認。未使用の依存やライブラリ内部の再試験は追加しない |
...
1. 変更の目的と対象を確認し、担当文書の契約と現在の実装を読む。
   Androidと関連Pythonでは、[29](29_Androidアプリ設計.md)の用途別採用表を先に確認し、既存APIへ任せる範囲と独自処理を残す理由を決める。
