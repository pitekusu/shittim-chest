...
| Androidの全記録同期（C31） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `RecordSyncCheckpoint.kt`、`RecordSynchronization.kt`、`RecordsRepository.kt`、`RecordSyncPanel.kt`、Circuit・Coroutines・Lifecycle Compose | 全ページ・重複・未取得詳細のみ取得、暗号化進捗・再オープン・キャンセル・cursor期限／巡回・404・認可喪失。開始／停止／再開・明暗・320dp／文字2倍。オフライン表示・更新統合はC32 |
...
| Androidの自動差分同期・アイコン保存（C32追加） | [29](29_Androidアプリ設計.md)、[16](16_セキュリティ・プライバシー詳細設計.md) | `RecordSyncWorker.kt`、`RecordSyncScheduler.kt`、既存暗号化Store、`read_api.py`・`http_api.py`・契約・RecordsApplicationの同期索引 | 本文の新規／更新分のみ取得、アイコン共用・暗号化・オフライン読込、Workerの期限／削除意図／本人検証、途中再開、全索引成功後の不在候補は404確認時だけ削除。新API先行配信後にAndroidを配布 |
