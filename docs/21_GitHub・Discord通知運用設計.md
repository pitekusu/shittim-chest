...
通知roleは作成しない。正常、Critical/High、CI failure、timeout、`action_required`、監視停止を含む全payloadを`allowed_mentions.parse=[]`とし、`content`と`allowed_mentions.roles`を設定しない。PR title、branch、commit message、GitHub userなどの外部入力はMarkdownとdisplay-control文字を無害化し、`@everyone`、`@here`、偽装linkを通知へ変換しない。
