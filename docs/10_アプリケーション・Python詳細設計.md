...
- STEP-07Aでは未送信outboxを新規送信しない。既存phase cancellationが永続checkpointを完了し、outbox drain/reconciliationはSTEP-07Bで追加する。
