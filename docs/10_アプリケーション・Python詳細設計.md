...
Runtimeは`stop_eligible_at`の5分前から、同じIDLE generationにつき最大1回だけ帰宅挨拶のlogical generationを開始し、Botが通常稼働中のまま通常text channelへ送信する。OpenAI application requestは最大2回・全体120秒、Discord送信は同じnonceで最大2回・全体60秒とし、retryableなtransport、timeout、429、5xx、未完了、citation不足、Structured Output不正だけを1回再試行する。生成後とDiscord再試行前に同じIDLE、generation、`stop_eligible_at`を再確認し、新規request、generation変更、IDLE解除、期限到達では結果を破棄する。SIGTERM後の挨拶送信は行わない。生成・送信・照合の失敗は安定codeと回数だけを記録して無視し、通常の90秒以内shutdownとscale-to-zeroを継続する。
