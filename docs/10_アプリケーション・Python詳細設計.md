...
- `runtime`: SDK非依存Protocolを通じてprocess signal、Discord readiness、task ownership、recoveryを統合する。
...
- process開始時は受付gateを閉じたまま4 Bot READYと必要なGuild Command syncを確認し、`resume_recoverable`を所有taskとして開始してから受付を開く。
- readinessは1秒周期で監視する。1 Botでも切断した時点で受付を即時停止し、60秒連続で復帰しなければinteraction-owned taskとstartup recovery taskをcancel・awaitして同一phaseを`CHECKPOINTED`へ保存する。通信断だけで`FAILED`へ遷移しない。
- 再接続時はCommandを再syncせず、4 READYを再確認して`resume_recoverable`を最大1 taskだけ再起動し、受付を再開する。owned taskの例外は受付を閉じてprocess failureとして伝播する。
...
- SIGINT/SIGTERM handlerはmain-thread event loopの`loop.add_signal_handler()`で登録し、callbackでは同期的に受付とinteraction dispatchを閉じてshutdown eventだけを設定する。同一signalの再入は冪等とする。
- graceful shutdownはreadiness monitorを停止し、interaction/recovery taskをcancel・awaitしてcheckpointし、controller、4 Discord clientの順にcloseする。アプリ内部deadlineは90秒、ECS `stopTimeout`は120秒とし、30秒をruntime/OS/log終了余裕として残す。90秒超過は`RuntimeShutdownTimeout`として明示的に失敗する。
...
- STEP-07Cの実process試験はSIGTERMで受付停止、checkpoint、interaction/client cleanupを確認し、SIGKILLでcleanupが行われない場合もreplacement processが永続active stateを検出してresumeできることを確認する。container runtimeとECS境界の強制終了はSTEP-08で追加する。
