...
- 想定実行基盤: Discord HTTP Interaction + AWS Lambda + ARM64 On-Demand ECS Fargate scale-to-zero
- 想定言語: Python 3.14.6（通常GIL build）
- 想定Discordライブラリ: discord.py
- 想定AI API: OpenAI Responses API
- 想定モデル: `gpt-5.6-luna`
- ステータス: local/CI実装・CDK synth検証済み、AWS/Discord未反映
- 確認基準日: 2026-07-28
...
| Ingress FIFO | HTTP Interactionを永続化し、実行Runtimeの起動まで順序付きで保持する最大20件の待機列 |
| Runtime Reconciler | 1分周期で永続状態とECS実状態を冪等に収束させるLambda |
...
DiscordIngress Lambda
  |
  +-- raw body Ed25519署名/timestamp検証
  +-- DynamoDB永続FIFO受付
  +-- Status Publisher / Runtime Reconciler起動要求
  |
  v
On-Demand Fargate Runtime (0→1, max 1)
  |
  v
DebateOrchestrator
  |
...
通常時はECS Serviceを`desiredCount=0`とし、Discordの署名済みHTTP InteractionをLambdaで永続受付した後だけ、4つのDiscord Botを1つのARM64 On-Demand Fargateタスク内で起動する。旧Fargate Spot専用・`desiredCount=1`常時稼働案は廃止済みである。
...
|
+-- RuntimeReconciler Lambda <-- EventBridge rate(1 minute)
|
+-- ECS Cluster
|    |
|    +-- ECS Service
|         |
|         +-- ARM64 On-Demand Fargate Task (desired 0, max 1)
|              |
|              +-- Python Container
|                   |
|                   +-- participant-aBot
...
| 項目 | 初期値 |
|---|---|
| ECSタスク数 | 通常0、稼働中最大1（On-Demand FARGATEのみ） |
| vCPU | 0.5 vCPU |
| メモリ | 1 GB |
| CPUアーキテクチャ | ARM64 |
| OS | Linux |
| desiredCount | 通常0、有効な作業がある場合のみ1へ収束 |
| stopTimeout | 120秒 |
| パブリックIP | 有効 |
| インバウンド通信 | 原則なし |
...
タスクはARM64の`512` CPU units / `1024` MiBに固定し、実測で不足が確認された場合だけ別計画で増強する。ECS Service Auto ScalingとFARGATE_SPOTは使用せず、maximum task countは1を維持する。
...
受付時にシッテムの箱は開始者のDiscord user IDに加え、当時のDiscord usernameとGuild上のdisplay nameを不変snapshotとして保存する。認可・識別の正本はuser IDだけとし、名前は将来の認証済みWeb討論アーカイブでの表示・検索に使う。Webアーカイブ自体は本MVPでは実装しない。後からDiscord上で名前が変わっても既存討論のsnapshotは更新しない。
...
ECS停止・異常終了はphaseではなく`recovery_state=checkpointed`として保存する。停止中の時間は討論の実処理時間へ加算せず、既に完了・保存した生成結果を再利用して未完了のphaseだけを再実行する。
...
DebateOrchestratorまたはEvidence Serviceが外部情報を1つのResponses API requestで取得し、同じEvidenceを3エージェントへ配布する。hosted tool内部のaction数は別途上限を設ける。
...
受付直後は依頼channelの公開Status Messageに起動状態を表示する。Runtime受理後は公開スレッド内の操作パネルへ現在phase、実処理経過時間、checkpoint/復元状態を表示する。3分時点の起動失敗表示は非terminalであり、15分まで自動復旧を続ける。
...
- システム全体: 最大3セッション
- Ingress FIFO: PENDING/CLAIMED/RETRYINGを合計し最大20件。accept済みの実行中討論は含めない
...
期限切れleaseなどの運用レコードにTTLを使用する場合も、即時解放をTTLへ依存せず、条件付き更新で無効化する。現行のDynamoDB item schemaはv7、scale-to-zeroで初期化するcontrol-record manifestはv2とし、直前versionからのみfail-closedで移行する。
...
Scale-to-zero停止中またはRuntime起動待ちの時間は討論の実処理時間へ加算しない。一方、Ingress Requestの3分/15分deadlineは受付時刻からの壁時計で判定する。
...
既存parameterを上書きせず、新しいversion pathを作成する。release/deploy manifestはparameter名とversionだけを保持し、値を読まない。DynamoDB control-record manifest v2と混同しない。各debateへconfig version、prompt hash、schema versionを保存し、prompt本文はlogへ出さない。
...
SIGTERM受信時はRuntimeの新規Ingress claimと討論開始を停止し、実行中のOpenAIリクエストをキャンセルする。HTTP Lambdaの永続受付はqueue上限まで継続する。完了済み成果物、現在フェーズ、active elapsed、outboxを保存し、Discordクライアントとログをflushして120秒以内に終了する。
...
Scale-to-zero停止中はBotのオフライン表示を許容する。有効なIngress RequestはDynamoDBに保持し、ReconcilerがOn-Demand Fargateを1タスクへ収束させ、READY/recovery完了後に未処理phaseから自動再開する。
...
- Discord HTTP InteractionはJSON parse前に生のraw body、Ed25519署名、timestampを検証する
- Interaction token、raw body、signatureは永続化も通常log出力もしない
...
0 tasks while idle; maximum 1 task while active
stopTimeout=120 seconds
```

旧FARGATE_SPOT専用・`desiredCount=1`案はscale-to-zero決定により廃止済みである。通常時はvCPU/memory課金を発生させず、永続受付後だけARM64 On-Demand Fargateを最大1タスク起動する。

AWSリソースへ`Project=shittim-chest`タグを付け、タグ対象の月額20 USD Budget通知を設定する。account全体のBudgetは月額30 USDとし、Cost Anomaly Detectionの異常総影響額thresholdは10 USDとする。OpenAI側にも月額50 USD通知を設定する。既存の手動10 USD AWS Budget/CAD通知は、CDK管理の新通知を作成・到達確認した後に撤去する。

Fargateの既定20 GiB ephemeral storageを使用し追加容量は設定しない。heartbeat用の`/tmp/shittim-chest`はephemeral storageではなく1 MiBのtmpfs mountとする。Container Insightsは無効とする。ECS標準のCPU・メモリ監視、EventBridgeによるtask停止通知、本文を含まない`BotReady`、heartbeat age、outbox滞留、失敗数の少数application metricとalarmで運用する。
...
- ECS Serviceは通常`desiredCount=0`、有効な作業がある間だけOn-Demand FARGATE 1タスク
- ECS task数は最大1とし、コンテナ異常終了またはlost wakeはReconcilerが再収束させる
- 停止中のBotオフライン表示とcold startを許容し、厳密なSLAは設けない
...
|---|---:|
| `/shittim`受付応答 | 3秒以内 |
| 起動状態公開 | HTTP受付後に非同期作成 |
| 討論開始 | 起動から3分を超えた場合は非terminal警告 |
| 討論完了目標 | 実処理180秒以内 |
| 討論完了ハード上限 | 実処理300秒以内 |
| 同時セッション | 3 |
| 通常時CPU | 70%未満 |
...
DiscordのInteractionには署名検証と永続受付後、HTTPで初回ephemeral応答を返す。ECSのGateway接続は初回応答の必須条件にしない。
...
1. HTTP ingressがraw body署名を検証し、依頼を永続FIFOに保存してephemeral応答する。Status Publisherが通常channelへ公開起動状態を作成し、RuntimeのREADY/recovery完了後に起点messageとPublic Threadを作成する。
...
- 通常時0・稼働中最大1のARM64 On-Demand ECS Fargateタスク
- Discord HTTP Interaction ingress、Status Publisher、Runtime Reconcilerの3 Lambda
- 永続FIFO 20件と実行中global 3 slot
- `/shittim question:<質問>`
...
- 決定事項
- DynamoDB保存
- 3分の非terminal起動警告、15分terminal failure、完全終了から30分後の自動停止
- 自動停止の2分前に帰宅挨拶を先行生成し、正常な30分IDLE停止時だけparticipant 1人が通常テキストチャンネルへ1件投稿するbest-effort通知
- checkpointと自動再開
- Discord outboxと重複抑止
- SSM Parameter Store
- CloudWatch Logs
...
- ECS Fargate
- API Gateway HTTP APIと3つのapplication Lambda
- ECR（完全immutable tag、既定暗号化、Managed Signing、OCI reference artifact）
- task definitionはECR tagではなく`repository@sha256:<digest>`を指定
- image digestへAWS Signer署名、SPDX SBOM、build provenance、vulnerability assessmentを紐付け、deploy前に自動検証
- SSM Parameter Store
- CloudWatch Logs
- DynamoDB
- タイムアウト
...
- On-Demand Fargate scale-to-zero、1分Reconciler、checkpoint/自動再開
...
- AI生成物を本人・専門家の見解として断定しない表現を維持する。仲間内限定運用ではterminal messageへ固定のAI免責文を表示しない
- 政治的質問への回答方針
...
- 1ユーザーによる連続実行を制限できる
- On-Demand FARGATEの`desiredCount=0`、maximum 1、ARM64 `512/1024`、`stopTimeout=120`をデプロイ定義で検証できる
- HTTP API、3 Lambda、1分Reconciler、FIFO 20、3/15/30分の境界を検証できる
...
- 東京リージョンのOn-Demand Fargate単価と実cold-start時間
...
| ECS | ARM64 On-Demand Fargate、通常`desiredCount=0`、稼働中最大1タスク |
| Discord受付 | API Gateway HTTP APIで署名検証し、Interaction tokenは保存しない |
| Lambda | Ingress、Status Publisher、Runtime Reconcilerの3つ |
| 起動/停止 | 3分非terminal警告、15分terminal、最後の完全終了から30分後に停止 |
| 帰宅挨拶 | 正常な30分IDLE停止時だけparticipant 1人が通常テキストチャンネルへ1件送る。生成・送信失敗は停止を妨げない |
| 待機/実行 | FIFO最大20件、accept後のglobal slot最大3件 |
| CPU | 0.5 vCPU |
| Memory | 1 GB |
| Architecture | ARM64 |
| stopTimeout | 120秒 |
...
| 予算通知 | Project tag 20 USD、account全体30 USD、OpenAI 50 USD。Cost Anomaly Detectionの異常総影響額threshold 10 USD。新CDK管理通知の確認後に旧手動10 USD通知を撤去 |
...
一方で、AWS上ではDiscord受付を3 Lambdaの軽量control planeへ分離し、討論runtimeは1つのARM64 On-Demand Fargateタスクへ統合する。通常時はscale to zeroとし、永続FIFOの依頼時だけ最大1タスクを起動する。チェックポイント、fencing、Outbox、Reconcilerで停止と競合から回復する。

現在の実装証跡はlocal test、DynamoDB Local、AWS SDK/Discord mock、CDK test、cdk-nag、synthである。AWS resourceは未deploy、Discord Application/Interactions Endpointは未変更、deployment guardは診断用library/CLIのみでproduction workflowには未接続である。
