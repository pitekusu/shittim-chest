...
Fargateの既定20 GiB ephemeral storageを使用し、heartbeat用の`/tmp` bind mountを含め追加容量は設定しない。Container Insightsは無効とする。ECS標準のCPU・メモリ監視、EventBridgeによるtask停止・Spot interruption通知、本文を含まない`BotReady`、heartbeat age、outbox滞留、失敗数の少数application metricとalarmで運用する。
