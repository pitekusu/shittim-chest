...
Fargateの既定20 GiB ephemeral storageを使用し追加容量は設定しない。heartbeat用の`/tmp/shittim-chest`はephemeral storageではなく1 MiBのtmpfs mountとする。Container Insightsは無効とする。ECS標準のCPU・メモリ監視、EventBridgeによるtask停止・Spot interruption通知、本文を含まない`BotReady`、heartbeat age、outbox滞留、失敗数の少数application metricとalarmで運用する。
