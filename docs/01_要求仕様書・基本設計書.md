...
AWSリソースへ`Project=shittim-chest`タグを付け、タグ対象の月額50 USD Budget通知を設定する。account全体のBudgetは月額30 USDとし、Cost Anomaly Detectionの通知thresholdも30 USDとする。OpenAI側にも月額50 USD通知を設定する。
...
| 予算通知 | Project tag 50 USD、account全体30 USD、OpenAI 50 USD。Cost Anomaly Detection threshold 30 USD |
