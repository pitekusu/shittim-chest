...
判断基準・候補は共有v9配下のレコード固有v1として追加する。試行の`deliberation_version`は新規1、
属性なしは従来0とし、未知版を拒否する。Recordsの読み手を先に配信してからCoreが新記録を書き始める。
共有スキーマ、制御記録11件、テーブル・索引の移行は不要。新記録作成後は未対応の旧版へ戻さず前方修正する。
