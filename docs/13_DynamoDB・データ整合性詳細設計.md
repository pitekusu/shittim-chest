...
| 記録 | 現行と読み込み互換性 |
|---|---|
| Core共有スキーマ | 未配信の現行v10。v8/v9を構造検証後にメモリ上で変換 |
| 討論実行の制御一覧 | 構成一覧（`manifest`）のスキーマv2 |
| 通常討論Outbox | レコードv2 |
| 元チャンネルへの親愛度Outbox | 投稿先を持つレコードv3。読込はv1〜v3に対応 |
| 公開状態の送信記録 | レコードv3 |
| 生成再開点・段階送信計画 | 共有v10系列の記録。固有版は変更しない |

共有v7以前、未知の将来版、未知のフィールド・列挙値は、黙って読み替えず処理を止める。
レコード固有の版と共有スキーマの版は区別する。
判断基準・候補は共有v10配下のレコード固有v1として保持する。試行の`deliberation_version`は新規2、
既存1と属性なしの従来0を維持し、未知版を拒否する。Recordsの読み手を先に配信してからCoreが新記録を書き始める。
v2追加のために共有10から版を上げたりテーブル・索引を追加したりしない。新記録作成後は未対応の旧版へ戻さず前方修正する。
Recordsの議事録テーブルの互換読込は[議事録設計](24_シッテムの箱%20議事録設計.md)を参照する。

親愛度の形式は共有版の「直前版」と切り離す。v8だけが旧識別子形式、v9/v10は匿名化キー形式とする。
Coreの読込・CAS、RecordsのAWS側Streamフィルター・handler入口・投影、メモリアルの読込・更新条件をこの区別に揃える。
メモリアルは読み取った版番号を更新条件に使用し、既存v9の保存形式を維持する。
過去議論の再投影はv8/v9/v10を受理するが、未知の版は拒否する。

Archive v3の読込は、投票先がその票の最高得点候補であることと採点コメントの一致も検証する。
同票なら両者からの総合得点を集計し、勝者と決定理由（多数決・総合得点・抽選）が一致することも検証する。
同点抽選用の非公開議論IDはArchiveへ保存しないため、同点内の確定済み投票先は維持する。

### 討論実行の制御記録の移行

配信前検査では、固定の制御記録11件が**すべて現行v10、またはすべて前版v9**であることを確認する。
...
`PreferenceFrame`と`CandidatePlan`はCoreの同一`DEBATE`／`ATTEMPT`配下で人格ごとに分離し、
本文と完了チェックポイントを同時保存する。保持期間は討論記録に従うが、再利用は同じ議論の復旧に限り、
質問者プロフィールや別の議論へ転用しない。Recordsの公開Archive・内容ハッシュ、Discord投稿、API、ログには含めない。

v2の進捗は試行メタデータの`candidate_coordination`／`opinion_reconsideration`へ保存する。
各進捗は工程、対象者とカーソル、選択待ちの候補（最大2件）、実行中の生成再開点を持つ。
候補・初回意見の更新と次の進捗を同じトランザクションへ含める。リース・世代・元の進捗を
比較更新の条件に含め、同じ更新時刻でも古い処理の上書きを拒否する。これらの進捗も公開投影しない。
...
Coreプロフィールの日時比較・更新は、Coreと同じ小数6桁・末尾`Z`のUTC文字列を使う。
同時刻の`+00:00`表記へ変換して条件式へ渡さない。既存のMemorial再開点の日時表記は変えず、
正本プロフィールとの境界だけで形式を合わせる。リセットの更新日時は既存プロフィールより過去へ戻さない。
...
画面を開き直した場合も、`queued`には本人による受付再送の導線を残す。再送は同じ仕事の配送であり、
新しい作品・成果物キー・有料試行枠を作らない。`generating`では再送操作を出さない。

HTTPの冪等キーとDynamoDBのトランザクショントークンは役割を分ける。
トークンはトランザクションの内容に結び付け、保存済み作品の回復が再び失敗した際に、
同じHTTPキーと異なる更新日時の組合せで再投入を妨げない。状態・試行番号のCASと成果物キーの固定を維持する。
...
同じトランザクションのリセット操作記録には、除去前の選出人格を `participant` として保存する。
ランキングの王冠はこの人格別に数え、質問者全体のリセット回数を3人格へ配らない。
