...
C26では将来の記録保存用秘密鍵をKeystoreで保護する部品だけを追加した。記録の永続保存はまだ行わない。
...
- Room・認証と記録Repositoryの接続はC28〜C30で行う。記録の永続保存やログアウト時の記録削除はまだ行わない。
...
- C27は保存用部品のみ。実際の議論本文の暗号化やRoom保存はC28以降で接続する。
