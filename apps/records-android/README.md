...
C26〜C28でKeystore・PQC・Roomの暗号化保存部品を追加し、C29で認証済み記録を保存するようにした。C30は期限付きオフライン認可とログアウト時の記録削除、C31は全記録同期・キャンセル・再開、C32は保存済み一覧・詳細のオフライン表示とオンライン更新を担当する。
...
- 認証済み一覧の「記録を端末に保存」から同期する。全一覧ページをたどり、未保存の詳細だけを1件ずつ暗号化保存する。件数未確定のため推測した割合は表示しない。
- 「一時停止」でキャンセルし、「同期を再開」で続きから取得する。詳細への移動・認証状態の変更・画面停止／再生成でも中断する。画面起点のCoroutinesと標準Lifecycle Composeを使い、WorkManagerやバックグラウンド継続を追加しない。
- 進捗は既存の暗号化Storeに別用途で保存し、Room schemaや既存本文の形式は変えない。ログアウト・別アカウントへの切替では進捗も消す。token・質問・プロフィールは進捗へ保存しない。
- 再起動後も次の開始操作で未完了分を続行する。cursor期限切れは1回だけ先頭から列挙し直し、保存済み詳細は再取得しない。通信／保存エラーやキャンセルでは完了扱いにしない。
- 完了後は「新しい記録も同期する」で再列挙する。C32では全ページ成功後に不在の保存済み記録を削除する。保存済み詳細の内容は開いた際に更新する。

## C32の保存済み表示と更新

- 前回の認可期限内なら、通信できない場合も保存済み一覧・詳細を閲覧できる。未取得の詳細は未保存と表示する。期限切れはロックし、同一アカウントの再認証で再び閲覧できる。ログアウト・切替は鍵・記録・進捗を消去する。
- 接続時は保存済み表示を先に出し、最新応答を暗号化保存して更新する。通信・端末保存失敗では既存の保存済み内容を残し、失敗と更新操作を表示する。オフラインのアイコンは代替表示で、古い署名付きURLへ接続しない。
- 認証済み404は一覧・詳細を同時に削除する。同期開始時の削除候補を暗号化進捗へ保存し、全ページ成功後だけ不在記録を削除する。失敗・キャンセルでは削除せず、再起動後も再開できる。C31の旧進捗は先頭から安全に再列挙する。既存Room schema・暗号形式は変えない。
- Play内部テスト：オンラインで「すべての記録を保存」を完了 → 機内モードで再起動し一覧・詳細を確認 → 再接続して更新 → ログアウト後は一覧・詳細が表示されないことを確認する。端末内のデータ削除は不要。
