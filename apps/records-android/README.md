...
C26〜C28でKeystore・PQC・Roomの暗号化保存部品を追加し、C29で認証済み記録を保存するようにした。C30は期限付きオフライン認可とログアウト時の記録削除、C31は全記録同期・キャンセル・再開を担当する。オフライン画面・オンライン更新統合はC32で接続する。
...
- 完了後は「新しい記録も同期する」で再列挙する。保存済み詳細の更新・削除反映とオフライン画面はC32で接続するため、この版はオフライン閲覧をまだ提供しない。
