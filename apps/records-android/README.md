...
C32追加の自動同期パネル（API 36、架空の画面状態のみ・実データなし）：
...
- ログイン・復帰時に自動同期し、「今すぐ差分を確認」でも予約できる。全索引ページをたどり、未保存・更新分だけを1件ずつ暗号化保存する。件数未確定のため推測した割合は表示しない。
...
- 初回保存後は軽量な同期索引を照合し、新規・更新分だけ取得する。C32では全ページ成功後に不在の保存済み記録を削除する。保存済み詳細は開くたびに再取得しない。
...
- ログイン・アプリ復帰後に即時同期を予約し、約15分間隔の定期同期を一意なWorkManagerジョブとして保持する。ネットワーク接続が必要で、省電力・Doze・強制停止等により実行は遅れる。サーバーpushによる瞬時反映ではない。「今すぐ差分を確認」で手動予約も可能。
