...
"""Register the private Inspector translation API key without echoing it."""
...
API_KEY_PARAMETER = f"{PARAMETER_ROOT}/openai/inspector-translation-api-key"
...
def parameter_names() -> tuple[str, ...]:
    """Return the exact Inspector translation parameter set."""
...
def collect_pending_setup(
    *,
    ...
    """Prompt only when the exact translation key is missing."""
    ...
    if API_KEY_PARAMETER in missing_parameters:
        value = _required_secret(secret_reader, "Inspector翻訳用OpenAI API key")
        confirmed = _required_secret(secret_reader, "Inspector翻訳用OpenAI API key (確認)")
...
def put_parameters(
    ...
    if not value or len(value.encode("utf-8")) > MAX_STANDARD_PARAMETER_BYTES:
...
def main(argv: Sequence[str] | None = None) -> int:
    """Check metadata or interactively register the translation API key."""
    ...
    try:
        ...
        if API_KEY_PARAMETER in metadata:
            ...
            _print_metadata(metadata)
            return 0
        print("翻訳用APIキーを対話入力します。値は画面・ログへ表示しません。")
        ...
        code = (
            ...
            else "records_inspector_translation_setup_failed"
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Records Inspector翻訳用OpenAI APIキーをSSMへ安全に登録する",
    )
