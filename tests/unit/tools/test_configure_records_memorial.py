"""Tests for the private Memorial generation API-key setup."""
...
from tools import configure_records_memorial as setup

PRIVATE_KEY = "private-memorial-generation-key"
...
def test_parameter_name_is_the_exact_private_memorial_path() -> None:
    assert setup.parameter_names() == ("/shittim-chest/production/records/openai/memorial-api-key",)
...
def test_metadata_output_never_prints_the_parameter_path(
    ...
    assert output == (
        "Records Memorial SSM SecureString: 1/1 設定済み (SecureString / Standard / version 2)\n"
    )
